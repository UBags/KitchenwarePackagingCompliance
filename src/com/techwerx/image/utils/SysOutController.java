...
public class SysOutController {
	...
	private static boolean writingToSysout = true;
		...
		setOut(new OutputStream() {
			...
			public void write(int b) throws IOException {
				if (b == '\n') { // if newLine
					...
					String pathData = "\u001B[37m" // low-visibility
							+ "\t :: (" + callerStEl.getFileName() + ":" + callerStEl.getLineNumber() + ") ["
							+ callerStEl + "]" // code path
							+ "\u001B[0m "; // reset
	...
	public static void redirectSysOut(File logFile) {
		PrintStream fileOut = null;
		try {
			fileOut = new PrintStream(logFile);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static boolean isWritingToSysout() {
		return writingToSysout;
	}
}